...
- **Decision**:
  1. A thread is *pending* when it is open and its last message is the user's (`isPendingComment`, shared by UI, server and the CLI's `--pending`). No schema change, and replies to agent-review findings count, so triaging findings feeds the same batch.
